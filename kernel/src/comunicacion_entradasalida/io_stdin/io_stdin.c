...
s_send_kernel_io_stdin_read_args send_kernel_io_stdin_read_args = {};
int recv_kernel_io_stdin_read_args = -1;
s_planificador_corto_args args_p_corto_io_stdin_test = {};

static void *handle_io_stdin_read_hilo(void *raw_args);
static bool recv_kernel_io_stdin_read(int fd_io);
static bool send_kernel_io_stdin_read(s_kernel_io_stdin_read msj, int fd_IO);
...
void handle_io_stdin_read(t_solicitud *solicitud, t_recurso *recurso)
{
    ...
    pthread_create(&io_communication, NULL, handle_io_stdin_read_hilo, (void *)args);
#ifndef TEST
...
static void *handle_io_stdin_read_hilo(void *raw_args)
{
    ...
    if (!send_kernel_io_stdin_read(solicitud->data_instruccion.stdin_read, recurso->fd_io))
    {
        log_trace(logger,
    ...
    if (!recv_kernel_io_stdin_read(recurso->fd_io))
    {
        log_trace(logger,
...
#else
        args_p_corto_io_stdin_test = args_p_corto;
#endif
    }
    else
    {
...
static bool send_kernel_io_stdin_read(s_kernel_io_stdin_read msj, int fd_IO)
{
    t_paquete *paquete_send = serializar_kernel_io_stdin_read(msj);
...
static bool send_kernel_io_stdin_read(s_kernel_io_stdin_read msj, int fd_IO)
{
    send_kernel_io_stdin_read_args.msj = msj;
    send_kernel_io_stdin_read_args.fd_IO = fd_IO;
    return true;
}
#endif
...
static bool recv_kernel_io_stdin_read(int fd_io)
{
    ...
    if (paquete->codigo_operacion != IO_KERNEL_STDIN_READ)
    {
        log_error(logger, "Estamos recibiendo en recv_kernel_io_stdin_read() un opcode raro: %i",
                  paquete->codigo_operacion);
        assert(!"recv_kernel_io_stdin_read() rompió");
    }
...
static bool recv_kernel_io_stdin_read(int fd_io)
{
    recv_kernel_io_stdin_read_args = fd_io;
    return true;
}
#endif