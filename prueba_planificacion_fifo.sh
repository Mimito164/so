...
# --------------- Test: Esperar a que empiece a ejecutar PLANI_4 (pid: 3) y ejecutar FINALIZAR_PROCESO con el PID de PLANI_4
echo """----------
Esperando que CPU ejecute el PID 3 para matarlo
---------- """
fin_pid_3=false
while ! $fin_pid_3 ; do
  
  if ! $fin_pid_3 && tmux capture-pane -p -t 2 -J | grep "PID: <3> - Estado Anterior: <READY> - Estado Actual: <EXEC>"; then
    fin_pid_3=true
  fi

done

sleep 1 # esperamos un rato antes de matar al proceso para comprobar los tests
...
echo """----------
Esperamos a que termine PID 1 y PID 2
---------- """
fin_p_1=false
while ! $fin_p_1 || ! $fin_p_2 ; do
  
  if ! $fin_p_1 && tmux capture-pane -p -t 2 | grep "Finaliza el proceso <1>"; then
    fin_p_1=true
  fi

  if ! $fin_p_2 && tmux capture-pane -p -t 2 | grep "Finaliza el proceso <2>"; then
    fin_p_2=true
  fi
